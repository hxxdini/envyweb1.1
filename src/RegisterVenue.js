...
export default function RegisterVenue() {
  ...
  const onSubmit = (e) => {
      ...
      const statRef = db.collection('Statistics').doc('Venues').collection(venueCountry).doc('RegisteredVenues');
      const managerRef = db.collection('Managers').doc(manager.uid);
      if(venueImage) {
            ...
            () => {
                uploadImage.snapshot.ref.getDownloadURL().then((url) =>{
                  ...
                  var batch = db.batch();
                    ...
                    batch.set(managerRef, {venueUID: venueRef.id}, {merge: true});
        ...
        <Form size='large' error key='large' loading={loading} onSubmit={onSubmit}>
          ...
          <Form.Field>
            <EditorWrapper>
              ...
              <EditorContainer>
                <Editor placeholder='Describe your venue' editorState={editorState} onChange={setEditorState} handleKeyCommand={handleKeyCommand} customStyleFn={customStyleFn}/>
              </EditorContainer>
            </EditorWrapper>
          </Form.Field>
          <Form.Field required>
          ...
          <Form.Group>
            ...
            <Form.Field width={7} required>
              ...
              <Input label={<Dropdown inline search defaultValue={phoneCode} options={phoneExt} value={phoneCode} onChange={handleCode}/>} labelPosition='left' name='venuePhone' value={values.venuePhone} onChange={handleChange('venuePhone')}/>
            </Form.Field>
          </Form.Group>
          <Divider/>
